//
//  CODUnbindPhoneViewController.m
//  YiJiaNet
//
...
@interface CODUnbindPhoneViewController () <UITextFieldDelegate>
...
@implementation CODUnbindPhoneViewController
...
//        make.width.offset(280*proportionW);
    ...
    [tipLabel SetlabTitle:@"温馨提示：验证已绑定手机号码之后即可绑定新手机号码" andFont:kFont(IS_IPHONE_5?11:13) andTitleColor:[UIColor grayColor] andTextAligment:NSTextAlignmentLeft andBgColor:nil];
    [self.view addSubview:tipLabel];
    [tipLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self.telNumTextField.mas_left);
        make.top.equalTo(self.verificationTextField.mas_bottom).offset(25);
    }];
    
    UIButton* registerBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    [registerBtn SetBtnTitle:@"解绑手机号" andTitleColor:[UIColor whiteColor] andFont:kFont(IS_IPHONE_5?13:15) andBgColor:CODColorTheme andBgImg:nil andImg:nil andClickEvent:@selector(confirmBtnAction) andAddVC:self];
    ...
    [registerBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.centerX.equalTo(self.view);
        make.top.equalTo(tipLabel.mas_bottom).offset(35*proportionW);
    }];
    
    self.telNumTextField.text = get(CODUserInfoKey)[@"mobile"];
    self.telNumTextField.userInteractionEnabled = NO;
}
...
//    }
    ...
    [[CODNetWorkManager shareManager] AFRequestData:@"m=App&c=member&a=check_code" andParameters:params Sucess:^(id object) {
        if ([object[@"code"] integerValue] == 200) {
            [SVProgressHUD cod_dismis];
            [self gotoBlindNewPhone];
