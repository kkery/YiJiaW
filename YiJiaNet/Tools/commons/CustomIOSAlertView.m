...
- (IBAction)customIOS7dialogButtonTouchUpInside:(id)sender
{
    ...
    int btnTag = [senderBtn tag];
    
    if (delegate != NULL)
    {
        [delegate customIOS7dialogButtonTouchUpInside:self clickedButtonAtIndex:btnTag];
    }

    if (onButtonTouchUpInside != NULL) {
        onButtonTouchUpInside(self, btnTag);
    }
}
...
- (void)addButtonsToView: (UIView *)container
{
    ...
    for (int i=0; i<[buttonTitles count]; i++)
    {
        ...
        [closeButton setFrame:CGRectMake(i * buttonWidth, container.bounds.size.height - buttonHeight, buttonWidth - 0.5, buttonHeight)];

        [closeButton addTarget:self action:@selector(customIOS7dialogButtonTouchUpInside:) forControlEvents:UIControlEventTouchUpInside];
        [closeButton setTag:i];

        [closeButton setTitle:[buttonTitles objectAtIndex:i] forState:UIControlStateNormal];
        [closeButton setTitleColor:CODColor333333 forState:UIControlStateNormal];
        [closeButton setTitleColor:CODColor333333 forState:UIControlStateHighlighted];
        ...
        {
            UIView *divideLine = [[UIView alloc] initWithFrame:CGRectMake(closeButton.frame.origin.x + closeButton.frame.size.width - 0.5, container.bounds.size.height - buttonHeight, 0.5, buttonHeight)];
            [divideLine setBackgroundColor:CODColorLine];
            [container addSubview:divideLine];
        }
    }
}
